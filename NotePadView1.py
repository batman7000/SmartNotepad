...
class Notepad:
    ...
    def set_status_bar(self):
        self.status_bar = ttk.Label(self.root, text='Status Bar',width=30)
        self.status_bar1=ttk.Label(self.root,text=f'Date:{str(date.today())}',width=30)
        self.status_bar.pack(side=tk.BOTTOM,padx=15)
        self.status_bar1.pack(side=tk.BOTTOM, padx=15)
        self.count = 0
    ...
    def exit_func(self,event=None):
        try:
            ...
            self.status_bar2 = ttk.Label(self.root, text=f'{self.end_time - start_time}')
            self.status_bar2.pack(side=tk.BOTTOM, anchor='center')
    ...
    def find_func(self, event=None):
        ...
        self.find_dialogue.resizable(False,False)
    ...
    def secure_file(self,event=None):
        if self.db_status:
            ...
            self.file_dialogue.resizable(False, False)
    ...
    def open_secure_file(self):
        try:
            ...
            if len(self.sel_file_index_tuple)==0:
                ...
                content,basename=file_controller.read_file(self.file_path)
                pwd=self.get_file_pwd()
            ...
            messagebox.showerror("Secure file Error","Error while securing file")
            print(traceback.format_exc())
    ...
    def remove_secure_file(self):
        try:
            self.sel_file_index_tuple=self.fileList.curselection()
            if len(self.sel_file_index_tuple!=0):
                self.full_file_name=self.fileList.get(self.sel_file_index_tuple)
                self.file_pos=self.full_file_name.find('[')
                self.file_name=self.full_file_name[:self.file_pos].strip()
    ...
    def mythread(self):
        Thread1=Thread(target=self.change_bold)
        Thread1.start()
        end_time1=time.time()
        self.exec_time=end_time1-start_time
